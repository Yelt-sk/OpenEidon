"""Single point of contact between Python and the Rust ``openeidon_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``openeidon_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    """Return the ``openeidon_rust`` module.
    ...
    """
    import openeidon_rust  # type: ignore[import-untyped]

    return openeidon_rust
...
def scan_result_from_json(json_str: str) -> object:
    ...
    from openeidon.security.types import (
        ScanFinding,
        ScanResult,
        ThreatLevel,
    )
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from openeidon.security.injection_scanner import (
        InjectionScanResult,
    )
    from openeidon.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from openeidon.tools.storage._stubs import RetrievalResult
