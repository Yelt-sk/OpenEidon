...
User configuration lives at ``~/.openeidon/config.toml``.  ``load_config()``
...
DEFAULT_CONFIG_DIR = Path.home() / ".openeidon"
DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "config.toml"


def _ensure_config_dir() -> Path:
    """Ensure the config directory exists with restrictive permissions."""
    from openeidon.security.file_utils import secure_mkdir
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.openeidon/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through OpenEidon. You are not a cloud service. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.openeidon/whatsapp_auth
    assistant_name: str = "Eidon"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "openeidon-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.openeidon/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.openeidon/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.openeidon/SOUL.md"
    memory_path: str = "~/.openeidon/MEMORY.md"
    user_path: str = "~/.openeidon/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.openeidon/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/openeidon/skill-index.git"
    index_dir: str = "~/.openeidon/skill-index/"
    max_depth: int = 5
...
@dataclass
class EidonConfig:
    """Top-level configuration for OpenEidon."""
...
# Sections that users may set via ``eidon config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(EidonConfig.__dataclass_fields__.keys()) - {"hardware"}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``EidonConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import openeidon.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "EidonConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> EidonConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``OPENEIDON_CONFIG`` when set,
        otherwise ``~/.openeidon/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = EidonConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("OPENEIDON_CONFIG"):
        config_path = Path(os.environ["OPENEIDON_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``eidon init``)
# ---------------------------------------------------------------------------
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.openeidon/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.openeidon/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.openeidon/whatsapp_auth
