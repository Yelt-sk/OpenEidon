"""Composition layer -- config-driven construction of a fully wired EidonSystem."""
...
from openeidon.core.config import EidonConfig, load_config
from openeidon.core.events import EventBus, get_event_bus
from openeidon.core.types import Message, Role
from openeidon.engine._stubs import InferenceEngine
from openeidon.tools._stubs import BaseTool, ToolExecutor
...
class EidonSystem:
    ...
    def ask(
        ...
        *,
        ...
        if context and self.memory_backend and self.config.agent.context_from_memory:
            try:
                from openeidon.tools.storage.context import (
                    ContextConfig,
                    inject_context,
                )
    ...
    def _run_agent(
        ...
        *,
        ...
        from openeidon.core.events import EventType
        from openeidon.core.registry import AgentRegistry
        ...
        if agent_name == "morning_digest" and hasattr(self.config, "digest"):
            ...
            from openeidon.tools.digest_collect import DigestCollectTool
            from openeidon.tools.text_to_speech import TextToSpeechTool
        ...
        try:
            if self.trace_store is not None:
                from openeidon.traces.collector import TraceCollector
        ...
        for name in tool_names:
            try:
                if name == "retrieval" and self.memory_backend:
                    from openeidon.tools.retrieval import RetrievalTool

                    tools.append(RetrievalTool(self.memory_backend))
                elif name == "llm":
                    from openeidon.tools.llm_tool import LLMTool
    ...
    def wire_channel(self, channel_bridge: Any) -> None:
        ...
        ----------
        channel_bridge:
            A connected :class:`~openeidon.channels._stubs.BaseChannel`
            instance whose ``on_message`` method accepts a callable.
        """
        from openeidon.core.types import Message, Role
        from openeidon.sessions.session import SessionStore
    ...
    def __enter__(self) -> EidonSystem:
        return self
...
class SystemBuilder:
    """Config-driven fluent builder for EidonSystem."""

    def __init__(
        self,
        config: Optional[EidonConfig] = None,
        *,
    ...
    def build(self) -> EidonSystem:
        """Construct a fully wired EidonSystem."""
        config = self._config
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            ...
            try:
                from openeidon.telemetry.energy_monitor import (
            ...
            if energy_monitor is None:
                try:
                    from openeidon.telemetry.gpu_monitor import GpuMonitor
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.openeidon/agents.db").expanduser()
                )
        ...
        if agent_manager is not None:
            try:
                from openeidon.agents.executor import AgentExecutor
                from openeidon.agents.scheduler import AgentScheduler
        ...
        # Transfer MCP clients so EidonSystem.close() can shut them down
    ...
    def _resolve_engine(self, config: EidonConfig):
        """Resolve the inference engine."""
        from openeidon.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: EidonConfig, engine: InferenceEngine) -> str:
    ...
    def _resolve_memory(self, config):
        """Resolve memory backend."""
        try:
            import openeidon.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        if not config.channel.enabled:
            return None
        try:
            import openeidon.channels  # noqa: F401 -- trigger registration
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from openeidon.sandbox.runner import ContainerRunner
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: EidonConfig):
        """Set up LearningOrchestrator when training is enabled."""
        if not config.learning.training_enabled:
            return None
        try:
            from openeidon.core.config import DEFAULT_CONFIG_DIR
            from openeidon.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from openeidon.learning.training.lora import LoRATrainingConfig
            from openeidon.traces.store import TraceStore
    ...
    def _discover_external_mcp(self, server_cfg) -> List[BaseTool]:
        ...
        from openeidon.mcp.transport import StdioTransport, StreamableHTTPTransport
        from openeidon.tools.mcp_adapter import MCPToolProvider
...
__all__ = ["EidonSystem", "SystemBuilder"]
