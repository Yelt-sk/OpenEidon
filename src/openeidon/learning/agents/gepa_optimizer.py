...
Uses GEPA's adapter pattern to bridge OpenEidon traces into GEPA's
...
from openeidon.core.config import GEPAOptimizerConfig
from openeidon.core.registry import LearningRegistry
from openeidon.learning._stubs import AgentLearningPolicy
...
class OpenEidonGEPAAdapter:
    """Implements GEPA's adapter protocol for OpenEidon agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'openeidon[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = OpenEidonGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "OpenEidonGEPAAdapter"]
