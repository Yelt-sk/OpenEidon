"""Eidon Agent backend — agent-level inference with tool calling."""
...
class EidonAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + EidonSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "eidon-agent"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. EidonConfig.agent.max_turns
