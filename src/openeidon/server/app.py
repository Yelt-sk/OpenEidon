"""FastAPI application factory for the OpenEidon API server."""
...
from openeidon.reminders import ReminderManager, set_default_reminder_manager
from openeidon.server.api_routes import include_all_routes
from openeidon.server.comparison import comparison_router
from openeidon.server.connectors_router import create_connectors_router
from openeidon.server.dashboard import dashboard_router
from openeidon.server.digest_routes import create_digest_router
from openeidon.server.routes import router
from openeidon.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from openeidon.server.channel_bridge import ChannelBridge
                    from openeidon.server.session_store import SessionStore
...
def create_app(
    ...
    *,
    ...
    config:
        Optional EidonConfig for other settings.
    """
    app = FastAPI(
        title="OpenEidon API",
        description="OpenAI-compatible API server for OpenEidon",
        version="0.1.0",
    )
    ...
    set_default_reminder_manager(reminder_manager)
    ...
    from openeidon.server.workflow_routes import workflow_router
    ...
    try:
        from openeidon.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from openeidon.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from openeidon.server.webhook_routes import (
    ...
    def _shutdown_reminders() -> None:
        mgr = getattr(app.state, "reminder_manager", None)
        if mgr is not None:
            mgr.stop()
