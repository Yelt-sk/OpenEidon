...
logger = logging.getLogger("openeidon.server.agent_manager")
...
class _LightweightSystem:
    ...
    full EidonSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    try:
        ...
        cfg = config
        if cfg is None:
            from openeidon.core.config import load_config
        ...
        try:
            from openeidon.core.events import get_event_bus
...
def _ensure_registries_populated() -> None:
    ...
    from openeidon.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import openeidon.channels  # noqa: F401
    except Exception:
        pass

    try:
        import openeidon.tools  # noqa: F401
    except Exception:
        pass

    # Also try to import browser tools (not included in openeidon.tools.__init__)
    for _browser_mod in ("openeidon.tools.browser", "openeidon.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("openeidon.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("openeidon.tools.")
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "openeidon.tools.browser",
            "openeidon.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from openeidon.core.credentials import TOOL_CREDENTIALS
    from openeidon.core.registry import ChannelRegistry, ToolRegistry
    ...
    try:
        for name, tool_cls in ToolRegistry.items():
            if name in _BROWSER_SUB_TOOLS:
                continue
            spec = None
            raw_spec = getattr(tool_cls, "spec", None)
            try:
                if isinstance(raw_spec, property):
                    spec = raw_spec.fget(tool_cls()) if raw_spec.fget else None
                elif callable(raw_spec):
                    spec = raw_spec(tool_cls)
                else:
                    spec = raw_spec
            except Exception:
                spec = None
...
def _build_deep_research_tools(
    ...
    from openeidon.tools.knowledge_sql import KnowledgeSQLTool
    from openeidon.tools.scan_chunks import ScanChunksTool
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from openeidon.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from openeidon.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from openeidon.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from openeidon.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from openeidon.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from openeidon.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from openeidon.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from openeidon.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from openeidon.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from openeidon.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from openeidon.core.events import EventType, get_event_bus
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.openeidon/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from openeidon.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from openeidon.core.credentials import get_credential_status
    ...
    async def sendblue_test(request: Request):
        ...
        try:
            payload: Dict[str, str] = {
                "number": to_number,
                "content": (
                    "Hello from your OpenEidon agent! "
