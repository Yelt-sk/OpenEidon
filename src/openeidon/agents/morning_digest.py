...
from openeidon.agents.digest_store import DigestArtifact, DigestStore
from openeidon.core.registry import AgentRegistry
from openeidon.core.types import Message, Role, ToolCall


def _load_persona(persona_name: str) -> str:
    """Load a persona prompt file by name."""
    search_paths = [
        Path("configs/openeidon/prompts/personas") / f"{persona_name}.md",
        Path.home() / ".openeidon" / "prompts" / "personas" / f"{persona_name}.md",
    ]
...
class MorningDigestAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._persona = kwargs.pop("persona", "eidon")
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        evaluator_feedback = ""
        try:
            from openeidon.agents.digest_evaluator import DigestEvaluator
