...
from openeidon.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a EidonSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after EidonSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from openeidon.agents import AgentRegistry
        ...
        # Resolve engine + model from EidonSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in EidonSystem")
        ...
        if router_policy_key and self._system:
            try:
                from openeidon.core.registry import RouterPolicyRegistry
                from openeidon.learning.routing.types import (
        ...
        if tool_names:
            try:
                ...
                _BROWSER_SUB_TOOLS = set()
            from openeidon.core.registry import ToolRegistry

            expanded_tool_names: list[str] = []
            for tname in tool_names:
                if tname == "browser":
                    expanded_tool_names.extend(sorted(_BROWSER_SUB_TOOLS))
                else:
                    expanded_tool_names.append(tname)

            for tname in expanded_tool_names:
