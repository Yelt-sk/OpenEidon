...
Recipes are the universal composition format for OpenEidon.  Each recipe
...
_USER_RECIPES_DIR = Path.home() / ".openeidon" / "recipes"
_USER_OPERATORS_DIR = Path.home() / ".openeidon" / "operators"
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Eidon.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Eidon constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
    ...
    def to_eval_suite(
        ...
        """
        from openeidon.recipes.composer import recipe_to_eval_suite
    ...
    def to_operator_manifest(self) -> Any:
        ...
        from openeidon.recipes.composer import recipe_to_operator
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.openeidon/recipes/`` directory
    4. User ``~/.openeidon/operators/`` directory
