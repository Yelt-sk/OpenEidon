...
class OAuthProvider:
    ...
    # Filenames in ~/.openeidon/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.openeidon/connectors/`` and falls
    back to environment variables ``OPENEIDON_{NAME}_CLIENT_ID`` and
    ``OPENEIDON_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"OPENEIDON_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to OpenEidon.</p>"
                    b"</body></html>"
                )
