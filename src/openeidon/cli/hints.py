...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]eidon init[/bold] to detect hardware and generate "
        "[cyan]~/.openeidon/config.toml[/cyan].\n"
        "  Or run [bold]eidon quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]eidon doctor[/bold] to check all engines.\n"
        "  Run [bold]eidon quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]eidon config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]eidon model list[/bold] to see available models."
        )
    return (
        "[yellow]Hint:[/yellow] No models available.\n"
        "  Pull a model first: [bold]ollama pull qwen3:4b[/bold]\n"
        "  Run [bold]eidon model list[/bold] to see available models."
    )
