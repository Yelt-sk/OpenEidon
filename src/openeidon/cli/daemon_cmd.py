"""``eidon start|stop|restart|status`` — daemon management commands."""
...
from openeidon.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the OpenEidon server daemon."""
...
def start(
    ...
    """Start the OpenEidon server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'eidon stop' to stop it first, or 'eidon restart'.")
        sys.exit(1)
    ...
    # Build command to run eidon serve
    cmd = [sys.executable, "-m", "openeidon.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]OpenEidon server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running OpenEidon server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the OpenEidon server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the OpenEidon server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
