"""``eidon agents`` — persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: eidon agents create[/dim]"
            )
            return
...
def _get_system():
    """Build a EidonSystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a EidonSystem instance."""
    if system is None:
        system = _get_system()
...
def launch():
    ...
    from openeidon.agents.manager import AgentManager as _AM
