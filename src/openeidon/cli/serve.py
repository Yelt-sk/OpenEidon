"""``eidon serve`` — OpenAI-compatible API server."""
...
from openeidon.engine import (
    discover_engines,
    discover_models,
    get_engine,
)
from openeidon.intelligence import (
...
def serve(
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import openeidon.tools  # noqa: F401  # trigger registration
                    from openeidon.core.registry import ToolRegistry
                    from openeidon.tools._stubs import BaseTool
    ...
    api_key = _os.environ.get("OPENEIDON_API_KEY", "")
    if not api_key:
        try:
            import tomllib

            _cfg_path = str(
                __import__("pathlib").Path.home() / ".openeidon" / "config.toml"
            )
    ...
    from openeidon.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from openeidon.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting OpenEidon API server[/green]\n"
