"""Check for newer OpenEidon releases on GitHub."""
...
_CACHE_PATH = Path("~/.openeidon/version-check.json").expanduser()
_CACHE_TTL = 86400  # 24 hours
_GITHUB_API = "https://api.github.com/repos/open-eidon/OpenEidon/releases/latest"
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            sys.stderr.write(
                f"\033[33mA new version of OpenEidon is available "
                f"(v{current} \u2192 v{latest})\n"
                f"Update: cd ~/OpenEidon && git pull && uv sync\033[0m\n\n"
            )
