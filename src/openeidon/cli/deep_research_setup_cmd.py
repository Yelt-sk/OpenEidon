"""``eidon deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from openeidon.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from openeidon.connectors.imessage import IMessageConnector

        return IMessageConnector(db_path=config.get("db_path", ""))
    elif connector_id == "obsidian":
        from openeidon.connectors.obsidian import ObsidianConnector

        return ObsidianConnector(vault_path=config.get("vault_path", ""))
    elif connector_id == "gmail_imap":
        from openeidon.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from openeidon.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from openeidon.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from openeidon.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from openeidon.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.openeidon/sync_state.db`` when empty.
