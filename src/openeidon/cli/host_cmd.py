"""``eidon host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      eidon host mlx-community/Qwen2.5-7B-4bit --backend mlx
      eidon host Qwen/Qwen3-8B --backend vllm
      eidon host qwen3:8b --backend ollama
      eidon host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]OpenEidon will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
