"""``eidon doctor`` — run diagnostic checks on the OpenEidon installation."""
...
from openeidon.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `eidon init` to generate a config file.",
    )
...
def _check_engines() -> List[CheckResult]:
    ...
    from openeidon.engine import _discovery
                ...
                if models:
                    ...
                    results.append(
                        CheckResult(
                            ...
                            "warn",
                            "No models available",
                            details="Pull a model (e.g. `ollama pull qwen3:4b`).",
                        )
                    )
        except Exception:
            continue
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "openeidon[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "openeidon[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "openeidon[energy-amd]", "AMD energy monitoring"),
        ("colbert", "openeidon[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "openeidon[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your OpenEidon installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]OpenEidon Doctor[/bold]")
    console.print()
