"""``eidon bench`` — run inference benchmarks."""
...
def run(
    ...
    if needs_energy and energy_monitor is None:
        ...
        extra_hint = (
            "openeidon[energy-apple]"
            if is_darwin_arm
            else "openeidon[gpu-metrics]"
            if platform.system() == "Linux"
            else "openeidon[energy-all]"
        )
...
def skills(
    ...
    from openeidon.evals.skill_benchmark import (
