...
def _get_discovered_dir() -> Path:
    ...
    return Path("~/.openeidon/skills/discovered/").expanduser()


def _get_overlay_dir() -> Path:
    """Return the directory where optimization overlays are stored."""
    return Path("~/.openeidon/learning/skills/").expanduser()
...
def _get_skill_paths() -> List[Path]:
    ...
    user_dir = Path("~/.openeidon/skills/").expanduser()
...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from openeidon.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from openeidon.skills.sources.openclaw import OpenClawResolver
    ...
    if source == "github":
        ...
        cache = _Path(
            "~/.openeidon/skill-cache/github/" + url.rstrip("/").rsplit("/", 1)[-1]
        ).expanduser()
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``eidon skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from openeidon.skills.importer import SkillImporter
    from openeidon.skills.parser import SkillParser
    from openeidon.skills.tool_translator import ToolTranslator
...
def search(query: str):
    ...
    Console().print(
        ...
        "Run 'eidon skill update' first.[/yellow]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.openeidon/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="openeidon-discover-dryrun-"))
        try:
