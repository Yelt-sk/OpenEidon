"""``eidon init`` — detect hardware, generate config, write to disk."""
...
from openeidon.cli.model import find_model_spec, hf_download, ollama_pull
...
# Engines supported by ``eidon init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     eidon ask "Hello"\n'
            "\n"
            "  Run `eidon doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]eidon scan[/cyan] for a full environment audit.")
...
def init(
    ...
    """Detect hardware and generate ~/.openeidon/config.toml."""
    console = Console()
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "openeidon" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]eidon doctor[/bold] to verify."
        )
        return
...
"""
        ...
        console.print(
            ...
            "Run [bold]eidon connect gdrive[/bold] to connect "
            "Google services, then [bold]eidon digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Eidon, a helpful personal AI assistant.\n"
        )
    ...
    if not model:
        ...
        from openeidon.core.config import _available_memory_gb
        ...
        if not no_download and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]eidon model pull {model}[/bold]"
                )
