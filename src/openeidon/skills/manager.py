...
from openeidon.skills.dependency import validate_dependencies
from openeidon.skills.executor import SkillExecutor, SkillResult
from openeidon.skills.loader import discover_skills
from openeidon.skills.tool_adapter import SkillTool
from openeidon.skills.types import SkillManifest
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.openeidon/learning/skills/ if config can't be loaded.
            try:
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.openeidon.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from openeidon.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("openeidon", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["openeidon"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.openeidon.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("openeidon", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.openeidon/skills/discovered/``).  Returns a list of dicts with
        ...
        if output_dir is None:
            output_dir = Path("~/.openeidon/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "openeidon (auto-discovered)"')
