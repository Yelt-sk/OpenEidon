...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.openeidon.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "openeidon_meta" the value is
# stored under manifest.metadata["openeidon"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("openeidon_meta", "platforms"),
    "prerequisites": ("openeidon_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.openeidon.original_frontmatter,
        and merges metadata.openeidon.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize openeidon namespace
        oj_meta = dict(raw_metadata.get("openeidon") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.openeidon.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.openeidon.* into canonical fields (these override
        # top-level mappings since they are explicit OpenEidon-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.openeidon.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the openeidon metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["openeidon"] = oj_meta
