...
function SettingsPanel() {
  ...
  const handleExport = () => {
    const blob = new Blob([localStorage.getItem('openjarvis-conversations') || '{}'], { type: 'application/json' });
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.version === 1) { localStorage.setItem('openjarvis-conversations', JSON.stringify(data)); useAppStore.getState().loadConversations(); flash(); }
        } catch {}
      };
  ...
  const handleClear = () => {
    ...
    localStorage.removeItem('openjarvis-conversations'); useAppStore.getState().loadConversations();
