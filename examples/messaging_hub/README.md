...
OpenEidon orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting OpenEidon to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- OpenEidon installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the OpenEidon channel docs).
...
OpenEidon supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/openeidon/config.toml` for the full list of channel and agent
options.
...
this via the OpenEidon scheduler:

```bash
eidon scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/openeidon/recipes/data/operators/` for examples.
