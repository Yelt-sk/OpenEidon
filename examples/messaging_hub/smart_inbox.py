...
def _run_demo(model: str, engine_key: str) -> None:
    ...
    try:
        ...
        click.echo(
            "Error: openeidon is not installed. "
    ...
    try:
        j = Eidon(model=model, engine_key=engine_key)
    except Exception as exc:
        click.echo(
            f"Error: could not initialize Eidon — {exc}\n\n"
...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: eidon channel list   to see available channels.\n",
    )
    ...
    click.echo("  from openeidon import Eidon")
    click.echo(f'  j = Eidon(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: eidon channel status  (to verify "{channel}" is connected)')
