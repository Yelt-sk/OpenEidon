...
A set of developer-focused scripts that use OpenEidon tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Eidon` instance with the `native_react` (ReAct) agent
...
1. **Install OpenEidon** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/openeidon/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/openeidon/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run eidon eval list` or
inspect `src/openeidon/tools/` for the full registry.
