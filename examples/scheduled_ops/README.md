...
This tutorial demonstrates how to use OpenEidon to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Eidon` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `eidon scheduler` CLI
...
Register each script as a recurring task with `eidon scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | eidon channel send slack
...
See `eidon channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let OpenEidon auto-select from what is available.
