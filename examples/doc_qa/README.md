...
Index a directory of documents into OpenEidon memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- OpenEidon installed (`git clone https://github.com/Yelt-sk/OpenEidon.git && cd OpenEidon && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `Eidon.memory.index()` to chunk the documents at
...
OpenEidon SDK. Adjust `--chunk-size` and `--top-k` to tune the
