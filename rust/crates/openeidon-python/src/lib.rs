//! PyO3 bridge — exposes ~50 Rust classes to Python via `openeidon_rust`.
...
fn load_config(path: Option<&str>) -> PyResult<core::PyConfig> {
    ...
    let config = openeidon_core::load_config(p)
...
fn detect_hardware() -> PyResult<String> {
    let hw = openeidon_core::hardware::detect_hardware();
    Ok(serde_json::to_string(&hw).unwrap_or_default())
}

#[pyfunction]
fn check_ssrf(url: &str) -> Option<String> {
    openeidon_security::check_ssrf(url)
}

#[pyfunction]
fn is_sensitive_file(path: &str) -> bool {
    openeidon_security::is_sensitive_file(std::path::Path::new(path))
}

#[pyfunction]
fn register_builtin_models() {
    openeidon_core::model_catalog::register_builtin_models();
}

#[pyfunction]
fn classify_query(query: &str) -> &'static str {
    openeidon_learning::classify_query(query)
}

#[pymodule]
fn openeidon_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // --- Core types ---
