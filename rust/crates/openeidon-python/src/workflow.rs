...
pub struct PyWorkflowGraph {
    inner: openeidon_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: openeidon_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => openeidon_workflow::NodeType::Tool,
            "condition" => openeidon_workflow::NodeType::Condition,
            "parallel" => openeidon_workflow::NodeType::Parallel,
            "loop" => openeidon_workflow::NodeType::Loop,
            "transform" => openeidon_workflow::NodeType::Transform,
            _ => openeidon_workflow::NodeType::Agent,
        };
        let node = openeidon_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = openeidon_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<openeidon_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(openeidon_workflow::WorkflowBuilder::new(name)),
        }
    }
