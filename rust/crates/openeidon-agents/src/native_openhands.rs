...
use openeidon_core::{AgentContext, AgentResult, OpenEidonError, ToolResult};
use openeidon_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        openeidon_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        openeidon_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    OpenEidonError::Agent(openeidon_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<openeidon_engine::Engine>>;
