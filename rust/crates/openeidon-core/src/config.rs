...
//!
//! Rust translation of `src/openeidon/core/config.py`.
...
/// Default config directory: `~/.openeidon/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".openeidon")
}

/// Default config file path: `~/.openeidon/config.toml`
...
fn default_sandbox_image() -> String { "openeidon-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.openeidon/operators".into() }
...
// ---------------------------------------------------------------------------
// Top-level EidonConfig
// ---------------------------------------------------------------------------

/// Top-level configuration for OpenEidon.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct EidonConfig {
    #[serde(skip)]
...
pub fn load_config(path: Option<&Path>) -> Result<EidonConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: EidonConfig = toml::from_str(&content)?;
        ...
        let mut cfg = EidonConfig::default();
        cfg.engine.default = recommended_engine;
        cfg
    };
...
mode = "block"
"#;
        let cfg: EidonConfig = toml::from_str(toml_str).unwrap();
