//! Error types for OpenEidon.

use thiserror::Error;

/// Top-level error type for all OpenEidon operations.
#[derive(Error, Debug)]
pub enum OpenEidonError {
...
mod tests {
    ...
    fn test_error_from_registry() {
        let e: OpenEidonError =
            RegistryError::DuplicateKey("foo".into(), "ToolRegistry").into();
        assert!(matches!(e, OpenEidonError::Registry(_)));
    }
