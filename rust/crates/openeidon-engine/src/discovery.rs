...
use openeidon_core::config::EidonConfig;
...
pub fn discover_engines(config: &EidonConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, OpenEidonError> {
    ...
    match key.as_str() {
        ...
        other => Err(OpenEidonError::Engine(
            openeidon_core::error::EngineError::ModelNotFound(format!(
                "Unknown engine: {}",
                other
            )),
        )),
...
mod tests {
    ...
    fn test_get_engine_static_ollama() {
        let config = EidonConfig::default();
