"""Tests for intent-based agent routing in EidonSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal EidonSystem instance for testing _detect_agent_intent."""
        from openeidon.system import EidonSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = EidonSystem.__new__(EidonSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("openeidon.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_eidon_triggers_digest(self, system):
        with patch("openeidon.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Eidon")
