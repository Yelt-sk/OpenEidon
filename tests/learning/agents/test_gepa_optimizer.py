...
class TestGEPAOptimizerOptimize:
    ...
    def test_no_gepa_reports_error(self) -> None:
        ...
        with patch.dict("sys.modules", {"gepa": None}):
            with patch(
                "openeidon.learning.agents.gepa_optimizer.HAS_GEPA",
                False,
            ):
                result = optimizer.optimize(mock_store)
...
class TestOpenEidonGEPAAdapter:
    def test_adapter_init(self) -> None:
        from openeidon.core.config import GEPAOptimizerConfig
        from openeidon.learning.agents.gepa_optimizer import (
            OpenEidonGEPAAdapter,
        )
