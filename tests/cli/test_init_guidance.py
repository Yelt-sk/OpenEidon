"""Tests for ``eidon init`` next-steps guidance."""
...
from openeidon.cli.init_cmd import _next_steps_text
...
class TestInitShowsNextSteps:
    def test_init_shows_next_steps(self, tmp_path: Path) -> None:
        ...
        config_dir = tmp_path / ".openeidon"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("openeidon.cli.init_cmd.PrivacyScanner"),
        ):
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "Getting Started" in result.output
        assert "eidon ask" in result.output
        assert "eidon doctor" in result.output
...
class TestMinimalConfig:
    def test_init_generates_minimal_by_default(self, tmp_path: Path) -> None:
        """Default eidon init produces a short config."""
        config_dir = tmp_path / ".openeidon"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
        ...
        assert "eidon init --full" in content

    def test_init_full_generates_verbose_config(self, tmp_path: Path) -> None:
        """eidon init --full produces the full reference config."""
        config_dir = tmp_path / ".openeidon"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
...
class TestInitEmptyModelFallback:
    def test_init_no_model_shows_warning(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("openeidon.cli.init_cmd.recommend_model", return_value=""),
            mock.patch("openeidon.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitDownloadDispatch:
    def test_init_ollama_download_calls_ollama_pull(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch(
                "openeidon.cli.init_cmd.ollama_pull",
                return_value=True,
            ) as mock_pull,
            mock.patch("openeidon.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitPrivacyHook:
    def test_init_shows_privacy_summary(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("openeidon.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("openeidon.cli.init_cmd.PrivacyScanner") as MockScanner,
        ):
            from openeidon.cli.scan_cmd import ScanResult
            ...
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "eidon scan" in result.output
