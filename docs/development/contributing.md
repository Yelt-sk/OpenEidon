...
```bash
uv run eidon --version   # Should print 0.1.0
uv run eidon --help      # Show all subcommands
```
...
OpenEidon uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=openeidon --cov-report=html
```
...
OpenEidon uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/openeidon/`:
...
src/openeidon/
    ...
    core/                       # Shared infrastructure
        config.py               # EidonConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # eidon ask
        init_cmd.py             # eidon init
        model.py                # eidon model list/info
        memory_cmd.py           # eidon memory index/search/stats
        telemetry_cmd.py        # eidon telemetry stats/export/clear
        bench_cmd.py            # eidon bench run
        serve.py                # eidon serve
```
...
try:
    import openeidon.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
...
- Absolute imports only (`from openeidon.core.registry import ...`)
