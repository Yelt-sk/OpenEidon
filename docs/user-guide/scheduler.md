...
!!! note "Optional component"
    The scheduler is a standalone module (`openeidon.scheduler`). It is not wired into the default `Eidon` / `EidonSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `eidon scheduler start`.
...
The `eidon scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
eidon scheduler list --status active

# Paused tasks
eidon scheduler list --status paused
```
...
eidon scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
eidon scheduler resume a3f9b12c4d8e
```
...
eidon scheduler cancel a3f9b12c4d8e
```
...
eidon scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.openeidon/scheduler.db")  # (1)!

# Wire in a EidonSystem for task execution
...
4. Starts a daemon thread named `"eidon-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.openeidon/config.toml`.

```toml title="~/.openeidon/config.toml"
[scheduler]
enabled = false
db_path = "~/.openeidon/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.openeidon/scheduler.db`     | SQLite database path                       |
