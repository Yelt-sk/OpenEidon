...
This writes a pre-configured `~/.openeidon/config.toml` for the operative agent with scheduling support.
...
eidon scheduler status <task-id>

# Pause / resume / delete a task
eidon scheduler pause <task-id>
eidon scheduler resume <task-id>
eidon scheduler delete <task-id>

# Run a task immediately (outside its schedule)
eidon scheduler run <task-id>

# Stop the scheduler daemon
eidon scheduler stop
```
...
- **All data stays local**: State is stored in `~/.openeidon/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `eidon scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `eidon scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `eidon memory stats`. Clear old entries with `eidon memory clear --before 2026-01-01` if needed.
