...
OpenEidon ships with two benchmarks:
...
from openeidon.bench._stubs import BenchmarkResult
...
from openeidon.bench.latency import LatencyBenchmark
from openeidon.bench.throughput import ThroughputBenchmark
...
eidon bench run -n 50

# Run only the latency benchmark
eidon bench run -b latency

# Run only the throughput benchmark with 20 samples
eidon bench run -b throughput -n 20

# Specify model and engine
eidon bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
eidon bench run --json

# Write JSONL results to a file
eidon bench run -o results.jsonl

# Combine options
eidon bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from openeidon.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
eidon bench run -b context_length
```
