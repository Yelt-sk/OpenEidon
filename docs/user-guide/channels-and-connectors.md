...
OpenEidon has two types of integrations:
...
```json
{
    "display_information": { "name": "OpenEidon" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "OpenEidon", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "OpenEidon" |
...
Slack serves two purposes in OpenEidon:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [OpenEidon icon](https://github.com/Yelt-sk/OpenEidon/blob/main/assets/openeidon-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **OpenEidon** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "OpenEidon"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @OpenEidon` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @OpenEidon` in the channel |
...
3. **Connect in OpenEidon:**
   ...
   - CLI: `uv run eidon connect notion`
...
3. **Connect in OpenEidon:**
   ...
   - CLI: `uv run eidon connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the OpenEidon desktop app
...
3. **Connect in OpenEidon:**
   ...
   - CLI: `uv run eidon connect outlook`
...
2. **Connect in OpenEidon:**
   - Desktop/Browser: Agents → Channels tab → Obsidian → paste the vault path
   - CLI: `uv run eidon connect obsidian --path /path/to/vault`
...
4. **Connect in OpenEidon:**
   ...
   - CLI: `uv run eidon connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run eidon deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.openeidon/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.openeidon/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.

```
~/.openeidon/connectors/
