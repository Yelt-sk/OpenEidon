...
This writes a pre-configured `~/.openeidon/config.toml` for the code assistant.
...
```bash
eidon ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
eidon ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
eidon ask "Read main.py and explain the architecture"

# Fix a bug
eidon ask "Find and fix the bug in test_utils.py"

# Run tests
eidon ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
eidon ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.openeidon/config.toml`:
...
```bash
# Write a new script
eidon ask "Write a Python script that converts YAML to JSON"

# Explain existing code
eidon ask "Read src/openeidon/core/events.py and explain the EventBus pattern"

# Debug a failing test
eidon ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
eidon ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
eidon ask "Read src/openeidon/tools/calculator.py and write unit tests for it"

# Shell tasks
eidon ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`eidon chat`).
- For stronger isolation, use the sandboxed agent: `eidon ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `eidon`. Use `cd /path && command` in your prompt if needed, or run `eidon` from the project directory.
