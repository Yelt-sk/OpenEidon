...
The OpenEidon Python SDK provides a high-level interface for interacting with local inference engines, managing memory, and running agent workflows. The primary entry point is the `Eidon` class.
...
|---------------|------------------|---------|----------------------------------------------------------------|
| `config`      | `EidonConfig`   | `None`  | Provide a pre-built configuration object                       |
...
If no `config` or `config_path` is provided, the SDK loads configuration from the default location (`~/.openeidon/config.toml`), falling back to built-in defaults.
...
|-----------|----------------|-----------------------------------|
| `config`  | `EidonConfig` | The active configuration object   |
| `version` | `str`          | The OpenEidon version string     |
...
The `Eidon.memory` attribute provides a `MemoryHandle` for document indexing, search, and statistics. The memory backend is lazily initialized on first use.
...
Release all resources held by the `Eidon` instance, including the memory backend, telemetry store, and engine connection.
...
!!! tip "Context Manager Pattern"
    While `Eidon` does not implement `__enter__`/`__exit__` directly, you should always call `close()` when done to free database connections and other resources:
