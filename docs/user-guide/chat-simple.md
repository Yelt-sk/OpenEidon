...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible OpenEidon setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize OpenEidon

```bash
git clone https://github.com/Yelt-sk/OpenEidon.git
cd OpenEidon
uv sync
eidon init --preset chat-simple
```

### 3. Ask a question

```bash
eidon ask "What is quantum computing?"
```
...
```bash
# Single question
eidon ask "Explain the difference between TCP and UDP"
...
eidon ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
eidon ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
eidon ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.openeidon/config.toml` or override per-query:

```bash
eidon ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `eidon serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/Yelt-sk/OpenEidon/releases)
...
${EDITOR:-nano} ~/.openeidon/config.toml
...
eidon ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
OPENEIDON_MODEL=qwen3.5:9b eidon ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`eidon serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.
