...
- **Persistence:** SQLite database file (default: `~/.openeidon/memory.db`)
...
```python
from openeidon.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from openeidon.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `openeidon.tools.storage.*` imports.
...
[Source: docs/intro.md] OpenEidon is a modular AI framework...
...
eidon memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
eidon memory search "machine learning"
