...
| **Overlay** | Sidecar file at `~/.openeidon/learning/skills/` storing optimized descriptions and few-shot examples |
...
eidon skill install hermes:apple-notes

# Bulk install a category
eidon skill sync hermes --category research

# Run a skill directly
eidon skill run math-solver -a expression="41 + 82"

# See skill details
eidon skill info research-and-summarize
```
...
eidon skill sync hermes --category research
eidon skill sync hermes --category coding
eidon skill sync hermes  # everything (~150 skills)
```
...
eidon skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
eidon skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
eidon skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```

### Config-Driven Auto Import

Add sources to `~/.openeidon/config.toml` for automatic syncing:
...
OpenEidon can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
eidon skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.openeidon/skills/discovered/
eidon skill discover
```

Discovered skills land in `~/.openeidon/skills/discovered/` and automatically appear in `eidon skill list` on the next session.
...
eidon optimize skills --dry-run

# Run DSPy optimization
eidon optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
eidon optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
eidon skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.openeidon/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
eidon bench skills --max-samples 5 --seeds 42

# Single condition
eidon bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with OpenEidon | Implicit trust | Full access within declared capabilities |
...
```bash
eidon skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.openeidon/skills/"  # where skills are installed
...
overlay_dir = "~/.openeidon/learning/skills/"
```
...
2. **User** `~/.openeidon/skills/`
3. **Bundled** (shipped with OpenEidon)

## CLI Reference

| Command | Description |
|---------|-------------|
| `eidon skill list` | List installed skills |
| `eidon skill info <name>` | Show detailed skill information |
| `eidon skill run <name> [-a key=value]` | Execute a skill directly |
| `eidon skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `eidon skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `eidon skill sources` | List configured skill sources |
| `eidon skill update` | Pull latest from configured sources |
| `eidon skill remove <name>` | Remove an installed skill |
| `eidon skill search <query>` | Search the skill index |
| `eidon skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `eidon skill show-overlay <name>` | Inspect optimization output for a skill |
| `eidon optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `eidon bench skills [--condition C]` | Run the PinchBench skills benchmark |
