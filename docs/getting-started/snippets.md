---
title: Code Snippets
description: Copy-paste patterns for common OpenEidon tasks
---

# Code Snippets

Ready-to-use patterns for the most common OpenEidon tasks. Each snippet is self-contained and copy-pasteable.
...
Run with: `eidon compose run research_assistant "quantum computing advances"`

## API Server (1 command)

```bash
eidon serve --port 8000 --engine ollama --model qwen3:8b
```
...
```bash
docker build -t openeidon .
docker run -p 8000:8000 openeidon serve --host 0.0.0.0
```
