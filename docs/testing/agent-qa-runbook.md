...
| OpenEidon initialized | `uv run eidon doctor` all green |
| Rust extension built | `uv run maturin develop -m rust/crates/openeidon-python/Cargo.toml` |
| Desktop app running | `uv run eidon serve` + `cd frontend && npm run dev` |
...
|---|----------|-------|-----------------|------|
| 1 | Template launch | `eidon agents launch`, pick research_monitor | Agent created, config printed with curated tools | [ ] |
| 2 | Manual run | `eidon agents run <id>` | Output shows reasoning + tool calls, status -> idle | [ ] |
| 3 | Immediate ask | `eidon agents ask <id> "summarize recent AI news"` | Synchronous response in terminal | [ ] |
| 4 | Queued instruct | `eidon agents instruct <id> "focus on diffusion"`, then `eidon agents run <id>` | Queued -> delivered, response in `eidon agents messages <id>` | [ ] |
| 5 | Status check | `eidon agents status` after 3+ runs | total_runs, total_cost, last_run_at populated | [ ] |
| 6 | Pause/resume | `eidon agents pause <id>`, verify skipped, `eidon agents resume <id>`, verify fires | Status toggles correctly | [ ] |
| 7 | Daemon scheduling | `eidon agents daemon` with interval agent (60s) | 3+ ticks fire on schedule, memory accumulates | [ ] |
| 8 | Budget exhaustion | Set max_cost=0.001, run until exceeded | Status becomes budget_exceeded | [ ] |
| 9 | Error recovery | Kill Ollama mid-tick, then `eidon agents recover <id>` | Error -> recover -> idle with checkpoint | [ ] |
| 10 | Channel binding | `eidon agents bind <id> --slack #test`, run tick | Agent sends to Slack | [ ] |
| 11 | Multi-agent | Launch 3 agents, different intervals, run daemon | All fire independently | [ ] |
| 12 | Template tools | Create from each template, `eidon agents info <id>` | All curated default tools listed | [ ] |
