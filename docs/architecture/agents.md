...
!!! note "Backward compatibility"
    The old `from openeidon.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.
...
1. On first call, copies the bundled `claude_code_runner/` to `~/.openeidon/claude_code_runner/` and runs `npm install --production` if `node_modules` is absent
...
4. Reads stdout and extracts the JSON payload between `---OPENEIDON_OUTPUT_START---` and `---OPENEIDON_OUTPUT_END---` sentinels
...
- `cleanup_orphans()` removes any stale containers labelled `openeidon-sandbox=true`
