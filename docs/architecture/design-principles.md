...
OpenEidon follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in OpenEidon is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
OpenEidon is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Eidon(engine_key="ollama")  # Local Ollama server
...
OpenEidon **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `eidon init` and used as the default engine:

```bash
eidon init --force
# Detects hardware, writes ~/.openeidon/config.toml with:
# [engine]
...
```bash
eidon telemetry stats          # Aggregated statistics
eidon telemetry export --json  # Export all records
```
...
OpenEidon provides a **clean Python API** through the `Eidon` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`openeidon[server]`, `openeidon[memory-colbert]`, etc.)
...
The API server (`eidon serve`) implements the **OpenAI chat completions API format**, making OpenEidon a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to OpenEidon:
...
OpenEidon requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of OpenEidon.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `openeidon[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `openeidon[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `openeidon[memory-bm25]` | `rank-bm25` |
| API server | `openeidon[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `openeidon[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `openeidon[inference-vllm]` | `vllm` |
| PDF ingestion | `openeidon[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `openeidon[channel-whatsapp-baileys]` | Node.js 22+ |
