...
```python
from openeidon.core.registry import ModelRegistry
...
```python
from openeidon.intelligence import merge_discovered_models
...
The following names are still importable from `openeidon.intelligence` via shim modules, but their canonical locations have moved:
