...
- **Persistence:** Data persists across restarts in `~/.openeidon/memory.db`
...
from openeidon.tools.storage.faiss_backend import FAISSMemory
from openeidon.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from openeidon.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `openeidon.tools.storage.*`.
...
```python
from openeidon.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from openeidon.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from openeidon.tools.storage.context import inject_context, ContextConfig
...
from openeidon.tools.storage._stubs import MemoryBackend
...
The default backend is configured in `~/.openeidon/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
